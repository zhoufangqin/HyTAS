...
class AttentionSuper(nn.Module):
    def __init__(self, super_embed_dim, num_heads=8, local_attn=0, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., normalization = False, relative_position = False,
                 num_patches = None, max_relative_position=14, scale=False, change_qkv = False, return_attn=False):
        super().__init__()
        ...
        self.attn_drop = nn.Dropout(attn_drop) #args.drop
        self.proj_drop = nn.Dropout(proj_drop) #args.drop

    def set_sample_config(self, sample_q_embed_dim=None, sample_num_heads=None, sample_local_attn=None, sample_in_embed_dim=None):
        self.sample_in_embed_dim = sample_in_embed_dim
        self.sample_num_heads = sample_num_heads
        self.sample_local_attn = sample_local_attn

        # if sample_local_attn == 1:
        #     self.sample_qk_embed_dim = sample_q_embed_dim
        #     self.sample_scale = (self.sample_qk_embed_dim // self.sample_num_heads) ** -0.5
        #     # self.proj.set_sample_config(sample_in_dim=self.sample_in_embed_dim, sample_out_dim=self.sample_qk_embed_dim)
        #     self.qkv.set_sample_config(sample_in_dim=self.sample_in_embed_dim, sample_out_dim=3 * self.sample_qk_embed_dim)
        #     # self.proj.set_sample_config(sample_in_dim=self.sample_in_embed_dim, sample_out_dim=self.sample_qk_embed_dim)
        #
        # else:
        if not self.change_qkv:
            self.sample_qk_embed_dim = self.super_embed_dim #why not self.super_embed_dim*self.super_num_heads?
        ...
        # print(self.change_qkv, self.sample_qk_embed_dim)
    ...
    def forward(self, x):
        B, N, C = x.shape #改了200
        qkv = self.qkv(x).reshape(B, N, 3, self.sample_num_heads, -1).permute(2, 0, 3, 1, 4) #shape '[32, 201, 3, 4, -1]' is invalid for input of size 1273536
        ## the embed_dim of super should be devided by 12 for heads(3,4,6)
