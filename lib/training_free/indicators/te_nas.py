## code from: https://github.com/lichuanx/T-CET/blob/master/ZeroShotProxy/compute_te_nas_score.py
...
def get_ntk_n(networks, recalbn=0, train_mode=False, num_batch=None,
    ...
    for i in range(num_batch):
        ...
        inputs = torch.randn(batch_size, 200, 147)
        for net_idx, network in enumerate(networks):
            ...
            for _idx in range(len(inputs_)):
                ...
                for layer in network.modules():
                    ...
                    elif isinstance(layer, nn.Linear) and layer.samples:
    ...
    for ntk in ntks:
        ...
        # L, V = torch.linalg.eigh(ntk, UPLO='U') #V: eigenvectors, L: eigenvalues
...
def compute_NTK_score( model, resolution=32, batch_size=16):
    gpu=0
    ntk_score = get_ntk_n([model], recalbn=0, train_mode=True, num_batch=1,
                           batch_size=batch_size, image_size=resolution, gpu=gpu)[0]
    print('ntk_score:', ntk_score)
    # rn_score = compute_RN_score(model=model, batch_size=batch_size, image_size=resolution,
    #                           num_batch=1, gpu=gpu)
    return -1 * ntk_score
    # return -1 * ntk_score + rn_score
